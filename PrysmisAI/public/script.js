document.addEventListener('DOMContentLoaded', () => {
    ...
    const processCode = async (action) => {
        ...
        try {
            let prompt;
            if(action === 'Obfuscate') {
                prompt = `Task: Highly Obfuscate this code. Use control flow flattening, dead code, and variable renaming. Output only raw code.\n\nCode:\n${code}`;
            } else {
                prompt = `Task: Deobfuscate and clean this code. Rename variables for clarity. Remove dead code. Output only raw code.\n\nCode:\n${code}`;
            }
