document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderHistory = () => {
        ...
        filtered.forEach(chat => {
            ...
            div.innerHTML = `<div class="font-medium text-white text-sm mb-1 truncate">${chat.title}</div><div class="text-[10px] text-gray-500">${new Date(chat.id).toLocaleDateString()}</div>`;
            div.onclick = () => {
                loadChat(chat.id);
                toggleHistory(false);
            };
        ...
        els.dumperFileInput.addEventListener('change', (e) => {
            if(e.target.files[0]) {
                ...
                reader.onload = (ev) => {
                    ...
                    logTerminal(`Loaded ${e.target.files[0].name}`);
                };
    ...
    const processCode = async (action) => {
        ...
        logTerminal(`Running ${action}...`);

        try {
            let prompt;
            if(action === 'Obfuscate') {
                prompt = `Act as a code obfuscator. Rewrite the following code to be incredibly difficult for humans to read but valid and executable. Use variable renaming, dead code injection, and logic flattening. Return ONLY the raw code.\n\nCode:\n${code}`;
            } else {
                prompt = `Act as a code deobfuscator. Analyze the following code. Rename variables to meaningful names based on their logic. Simplify control flow. Remove dead code. Return ONLY the raw, clean code.\n\nCode:\n${code}`;
            }
            ...
            if(data.candidates) {
                ...
                els.dumperAdviceArea.innerHTML = "Processing complete. Code available in output.";
    ...
    async function handleSend() {
        ...
        try {
            ...
            if(mode === 'History') sysPrompt = "You are a Historian. Provide context, dates, and causes/effects. Remain neutral and factual.";
            if(mode === 'Philosophy') sysPrompt = "You are a Philosopher. Explore the prompt using logic, ethics, and famous philosophical frameworks (Stoicism, Utilitarianism, etc).";
